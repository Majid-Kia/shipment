...
Audited against `docs/assignment/spec.md` after the final cleanup pass.
...
| ESLint                                                              | flat config plus `npm run lint`                            | Complete |
...
- Remaining production concern: the single bundle is approximately 526 kB
  minified (about 162 kB gzip), above Vite's warning threshold.

## Final scope accounting

All required functional behavior and named tooling are implemented. Optional
Playwright, profiler automation, deployment, and visual enhancements were not
added.
